...
#pragma omp parallel for simd
    ...
    if(rank == world_size-1){
      MPI_Sendrecv(&d_t[start_segment], 1, MPI_FLOAT, rank - 1, 0, &d_t[start_segment-1], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &stat);
    }else if(rank == 0){
      MPI_Sendrecv(&d_t[last_segment-1], 1, MPI_FLOAT, rank + 1, 0, &d_t[last_segment], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &stat);
    }else{
      MPI_Sendrecv(&d_t[last_segment-1], 1, MPI_FLOAT, rank + 1, 0, &d_t[start_segment-1], 1, MPI_FLOAT, rank - 1, 0, MPI_COMM_WORLD, &stat);
      MPI_Sendrecv(&d_t[start_segment], 1, MPI_FLOAT, rank - 1, 0, &d_t[last_segment], 1, MPI_FLOAT, rank + 1, 0, MPI_COMM_WORLD, &stat);
    }
  }
